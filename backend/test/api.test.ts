...
describe('Should test /getLocation', function () {
    ...
    it('success', async () => {
        window.fetch = jest.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ foo: "foo" }) })
    ...
    it('server  error', async () => {
        window.fetch = jest.fn().mockResolvedValue({ ok: false, json: () => Promise.resolve({ foo: "foo" }) })
        const res = await request(app).get('/getLocation').query({ ip: "" });
    ...
    it('bad ip  error', async () => {
        window.fetch = jest.fn().mockResolvedValue({ ok: false, json: () => Promise.resolve({ foo: "foo" }) })
        const res = await request(app).get('/getLocation').query({ ip: "dfg.sdf.sdf.sdf" });
    ...
    it('bad ip  error', async () => {
        window.fetch = jest.fn().mockResolvedValue({ ok: false, json: () => Promise.resolve({ foo: "foo" }) })
        const res = await request(app).get('/getLocation').set('x-forwarded-for', '').query({ ip: "" });
        expect(res.statusCode).toBe(400);
